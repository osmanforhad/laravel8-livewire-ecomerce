<main id="main">
    <div class="container">
        ...
        <!--On Sale-->
        @if (
        $onSaleProducts->count() > 0 
        && $onSaleTimer->status == 1 
        && $onSaleTimer->sale_date > Carbon\Carbon::now()
        )
        <div class="wrap-show-advance-info-box style-1 has-countdown">
            <h3 class="title-box">On Sale</h3>
            <div class="wrap-countdown mercado-countdown" 
            data-expire="{{Carbon\Carbon::parse($onSaleTimer->sale_date)->format('Y/m/d h:m:s')}}">
        </div>
            <div class="wrap-products slide-carousel owl-carousel style-nav-1 equal-container " data-items="5" data-loop="false" data-nav="true" data-dots="false" data-responsive='{"0":{"items":"1"},"480":{"items":"2"},"768":{"items":"3"},"992":{"items":"4"},"1200":{"items":"5"}}'>
                @foreach ($onSaleProducts as $onSaleProduct)
                <div class="product product-style-2 equal-elem ">
                    <div class="product-thumnail">
                        <a href="{{route('product.details', ['slug' => $onSaleProduct->slug])}}" title="{{$onSaleProduct->name}}">
                        <figure>
                        <img src="{{asset('assets/images/products')}}/{{$onSaleProduct->image}}" 
                        width="800" height="800" alt="{{$onSaleProduct->name}}">
                        </figure>
                        </a>
                    </div>
                    <div class="product-info">
                        <a href="{{route('product.details', ['slug' => $onSaleProduct->slug])}}" class="product-name">
                            <span>{{$onSaleProduct->name}}</span>
                        </a>
                        <div class="wrap-price">
                            <ins>
                                <p class="product-price">{{$onSaleProduct->sale_price}}$</p>
                            </ins> 
                            <del>
                                <p class="product-price">{{$onSaleProduct->regular_price}}$</p>
                            </del>
                        </div>
                    </div>
                </div>
                ...
                <div class="wrap-product-tab tab-style-1">
                    ...
                    <div class="tab-contents">
                        ...
                        <div class="tab-content-item {{$key==0 ? 'active' : ''}}" id="category_{{$category->id}}">
                            <div class="wrap-products slide-carousel owl-carousel style-nav-1 equal-container" data-items="5" data-loop="false" data-nav="true" data-dots="false" data-responsive='{"0":{"items":"1"},"480":{"items":"2"},"768":{"items":"3"},"992":{"items":"4"},"1200":{"items":"5"}}' >
                                @php
                                    //fetch product acording to category id
                                ...
                                <figure>
                                    ...
                                    <div class="product-info">
                                        <a href="{{route('product.details', ['slug' => $c_product->slug])}}" class="product-name">
